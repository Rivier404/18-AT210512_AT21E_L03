...
void ghi_file() {
    ...
    Sv* temp = HEAD;
    while (temp) {
        fwrite(temp, sizeof(Sv) - sizeof(Sv*), 1, f); 
        temp = temp->next;
    }
    fclose(f);
}
...
void doc_file() {
    ...
    while (fread(&sv, sizeof(Sv) - sizeof(Sv*), 1, f)) {
        printf("%-5d %-20s %-15d %10.2f\n", stt++, sv.Ho_ten, sv.Tuoi, sv.Diem_TB);
    }
    fclose(f);
}
...
int main() {
    ...
    while (1) {
        ...
        printf("3. In danh sach hien tai\n");
        printf("4. In danh sach tu tep (DSACH.C)\n");
        printf("5. Thoat\n");
